...
import { HomeCard, HomeCardGroup } from "./Skeletons";
...
function BusinessList() {
    ...
    if (Loading) return <HomeCardGroup />; // Corrected return logic

    return (
        <div>
            {Loading === false && count === 0 ? <TypographyH3 className="my-4">No Businesses found</TypographyH3> : null}

            {Loading === false && fetchType === "search" && (
                <Button
                    variant={"outline"}
                    onClick={() => {
            ...
            >
                {businessList.map((data: Business) => (
                    <Card key={data.id} className="gap-2 p-0">
                        <img
                            src={data.image || "/business-avatar.webp"}
                            alt={`${data.name} image`}
                            className="h-30 rounded-t-2xl object-center object-cover"
                        />
